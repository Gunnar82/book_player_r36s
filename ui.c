...
static void accent_load(void){
    ...
    while(fgets(line,sizeof(line),fp)){
        trim_line(line);if(!line[0]||line[0]=='#'||line[0]==';')continue;
        if(line[0]=='['){in_ui=!strcmp(line,"[ui]");continue;}if(!in_ui)continue;
        if(!strncmp(line,"accent_color=",13)){
            char *v=line+13;trim_line(v);for(int i=0;i<ACCENT_COUNT;i++)if(!strcasecmp(v,accents[i].key)||!strcasecmp(v,accents[i].name)){accent_index=i;break;}break;
        }
    }
    fclose(fp);
}
static int accent_save(void){
    const char *path=get_storage_config_path();FILE *fp=fopen(path,"r");char **lines=NULL;size_t count=0,cap=0;char line[1200];
    ...
    char tmp_path[1200];snprintf(tmp_path,sizeof(tmp_path),"%s.tmp",path);fp=fopen(tmp_path,"w");if(!fp)goto fail;
    int in_ui=0,have_ui=0,wrote=0;
    for(size_t i=0;i<count;i++){
        char check[1200];snprintf(check,sizeof(check),"%s",lines[i]);trim_line(check);
        if(check[0]=='['){if(in_ui&&!wrote){fprintf(fp,"accent_color=%s\n",accents[accent_index].key);wrote=1;}in_ui=!strcmp(check,"[ui]");if(in_ui)have_ui=1;fputs(lines[i],fp);continue;}
        if(in_ui&&!strncmp(check,"accent_color=",13)){fprintf(fp,"accent_color=%s\n",accents[accent_index].key);wrote=1;continue;}
        fputs(lines[i],fp);
    }
...
static SDL_Color substitute_accent(SDL_Color color){
    if((color.r==255&&color.g==220&&color.b==80)||(color.r==230&&color.g==210&&color.b==70)){SDL_Color a=ui_accent_color();a.a=color.a;return a;}return color;
}
...
{
    if (!font) return;
    color=substitute_accent(color);
    SDL_Surface *s = TTF_RenderUTF8_Blended(font, text, color);
    if (!s) return;
    SDL_Texture *t = SDL_CreateTextureFromSurface(r, s);
    if (t) { SDL_Rect dst = { x, y, s->w, s->h }; SDL_RenderCopy(r, t, NULL, &dst); SDL_DestroyTexture(t); }
    SDL_FreeSurface(s);
}
...
{
    if (!font) return;
    int w = 0, h = 0;if (TTF_SizeUTF8(font, text, &w, &h) != 0) return;
...
void format_time(double seconds, char *out, size_t size)
{
    ...
    int total = (int)seconds;int h = total / 3600;int m = (total % 3600) / 60;int s = total % 60;
    if (h) snprintf(out, size, "%d:%02d:%02d", h, m, s);else snprintf(out, size, "%02d:%02d", m, s);
}
