...
int main(int argc,char **argv)
{
 ...
 srand((unsigned int)time(NULL));app_log_clear();app_logf("Start %s %s",APP_NAME,APP_VERSION);setup_state_path();load_state();usage_app_starts++;load_playback_config();load_download_config();bluetooth_load_config();int bluetooth_present=bluetooth_adapter_present();if(bluetooth_present)bluetooth_autoconnect_start();else app_logf("Bluetooth: kein Adapter, Funktionen deaktiviert");
 ...
 while(running){
  ...
  if(locked&&lock_screen_visible_until&&SDL_GetTicks()<lock_screen_visible_until){draw_text(renderer,font,"Tastensperre",20,40,selected);draw_text(renderer,font,"Zum Entsperren:",20,100,white);int x=20;for(int i=0;i<UNLOCK_SEQUENCE_LEN;i++){const char *name=button_name(unlock_sequence[i]);SDL_Color col=i<unlock_progress?selected:white;draw_text(renderer,font,name,x,150,col);x+=70;}if(SDL_GetTicks()<unlock_wrong_flash_until)draw_text(renderer,font,"Falsche Taste",20,210,gray);draw_text(renderer,font,"5 s ohne Eingabe: zurueck zur Wiedergabe",20,270,gray);}else if(locked){
