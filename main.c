...
int main(int argc,char **argv)
{
 ...
 srand((unsigned int)time(NULL));app_log_clear();app_logf("Start %s %s",APP_NAME,APP_VERSION);setup_state_path();load_state();load_playback_config();load_download_config();bluetooth_load_config();int bluetooth_present=bluetooth_adapter_present();if(bluetooth_present)bluetooth_autoconnect_start();else app_logf("Bluetooth: kein Adapter, Funktionen deaktiviert");
 ...
 int sleep_timer_active=0,sleep_timer_minutes=SLEEP_DEFAULT_MINUTES;Uint32 sleep_timer_end_ticks=0;int shutdown_tracks_remaining=shutdown_after_tracks,shutdown_tracks_setting_seen=shutdown_after_tracks;int locked=0,unlock_sequence[UNLOCK_SEQUENCE_LEN],unlock_progress=0;Uint32 unlock_wrong_flash_until=0;MediaKeys media_keys;media_keys_init(&media_keys);MprisBridge mpris;memset(&mpris,0,sizeof(mpris));if(bluetooth_present)mpris_bridge_init(&mpris);HfpGateway *hfp_gateway=NULL;if(bluetooth_present&&hfp_gateway_init(&hfp_gateway)<0)app_logf("HFP IPC konnte nicht gestartet werden");Uint32 last_bluetooth_check=SDL_GetTicks();
 ...
 while(running){
  ...
  while(SDL_PollEvent(&e)){
   ...
   /* Analoge Joystick-Achsen absichtlich ignorieren. Steuerung nur ueber D-Pad/Buttons. */
   if(e.type==SDL_JOYAXISMOTION)continue;
  ...
  if(bluetooth_present)hfp_gateway_process(hfp_gateway);char hfp_number[64];MediaKeyAction hfp_action=MEDIA_KEY_NONE;if(bluetooth_present&&hfp_gateway_poll_dial(hfp_gateway,hfp_number,sizeof(hfp_number))){
  ...
  if(bluetooth_present)battery_bluez_process(battery_bluez);if(SDL_GetTicks()-last_battery_check>=5000){battery_percent=get_battery_percent();battery_charging=is_battery_charging();if(bluetooth_present)battery_bluez_set_percent(battery_bluez,battery_percent);last_battery_check=SDL_GetTicks();}
