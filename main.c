...
static const int LOCK_CANDIDATE_BUTTONS[] = { BUTTON_A, BUTTON_B, BUTTON_X, BUTTON_Y, BUTTON_L1, BUTTON_R1, BUTTON_L2, BUTTON_R2 };
static const char *LOCK_CANDIDATE_NAMES[] = { "A", "B", "X", "Y", "L1", "R1", "L2", "R2" };
...
{
    const char *audio_dir=NULL;if(argc>1&&argv[1][0])audio_dir=argv[1];
    srand((unsigned int)time(NULL));setup_state_path();load_state();load_playback_config();load_download_config();
    ...
    SDL_Window *window=SDL_CreateWindow("Hoerspiel Player",SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED,SCREEN_W,SCREEN_H,SDL_WINDOW_FULLSCREEN);
    ...
    init_backlight();init_battery();int battery_percent=get_battery_percent();int battery_charging=is_battery_charging();Uint32 last_battery_check=SDL_GetTicks();
    double cpu_usage=get_cpu_usage(),ram_usage=get_ram_usage(),cpu_temperature=get_cpu_temperature();Uint32 last_systemstats_check=SDL_GetTicks();
    SDL_Color white={255,255,255,255},selected={255,220,80,255},gray={160,160,160,255};
    char book_names[MAX_BOOKS][256],book_paths[MAX_BOOKS][512],book_roots[MAX_BOOKS][512];int book_track_counts[MAX_BOOKS];memset(book_track_counts,0,sizeof(book_track_counts));int book_count=0;
    StoragePath storage_paths[MAX_STORAGE_PATHS];int storage_path_count=get_storage_paths(storage_paths,MAX_STORAGE_PATHS);
    for(int si=0;si<storage_path_count&&book_count<MAX_BOOKS;si++){if(!storage_paths[si].available)continue;char names[MAX_BOOKS][256],paths[MAX_BOOKS][512];int count=scan_books_recursive(storage_paths[si].path,names,paths);for(int i=0;i<count&&book_count<MAX_BOOKS;i++){snprintf(book_names[book_count],sizeof(book_names[book_count]),"%s",names[i]);snprintf(book_paths[book_count],sizeof(book_paths[book_count]),"%s",paths[i]);snprintf(book_roots[book_count],sizeof(book_roots[book_count]),"%s",storage_paths[si].path);book_count++;}}
    for(int i=0;i<book_count;i++){Track tmp_tracks[MAX_TRACKS];book_track_counts[i]=scan_tracks(book_paths[i],tmp_tracks);}
    ...
    int sleep_timer_active=0,sleep_timer_minutes=SLEEP_DEFAULT_MINUTES;Uint32 sleep_timer_end_ticks=0;int shutdown_tracks_remaining=shutdown_after_tracks,shutdown_tracks_setting_seen=shutdown_after_tracks;
    int locked=0,unlock_sequence[UNLOCK_SEQUENCE_LEN],unlock_progress=0;Uint32 unlock_wrong_flash_until=0;MediaKeys media_keys;media_keys_init(&media_keys);

    while(running){
        SDL_Event e;
        while(SDL_PollEvent(&e)){
            if(e.type==SDL_QUIT){running=0;continue;}
            if(e.type==SDL_JOYBUTTONDOWN||e.type==SDL_KEYDOWN||(e.type==SDL_JOYAXISMOTION&&abs(e.jaxis.value)>AXIS_DEADZONE)){last_activity=SDL_GetTicks();if(e.type==SDL_JOYBUTTONDOWN||e.type==SDL_KEYDOWN){idle_timer_remaining_ms=idle_timer_minutes>0?(Uint32)idle_timer_minutes*60000U:0U;idle_timer_last_tick=SDL_GetTicks();}if(is_display_off()){set_display_off(0);continue;}}
            ...
            if(e.type==SDL_JOYBUTTONDOWN&&e.jbutton.button==BUTTON_SELECT){if(!locked){locked=1;generate_unlock_sequence(unlock_sequence,UNLOCK_SEQUENCE_LEN);unlock_progress=0;}continue;}
            if(locked){if(e.type==SDL_JOYBUTTONDOWN){if(e.jbutton.button==unlock_sequence[unlock_progress]){unlock_progress++;if(unlock_progress>=UNLOCK_SEQUENCE_LEN){locked=0;unlock_progress=0;}}else{unlock_progress=0;unlock_wrong_flash_until=SDL_GetTicks()+400;}}continue;}
            ...
            if(e.type==SDL_JOYBUTTONDOWN){
                if(screen!=SCREEN_DOWNLOADS&&e.jbutton.button==BUTTON_X){screen=SCREEN_SYSTEM_MENU;continue;}
                if(e.jbutton.button==BUTTON_START){if(!music&&track_count>0){music=play_track(tracks,track_index,base_position,&base_position,&started_ticks,&paused);if(music){duration=get_duration(music);last_save=SDL_GetTicks();}}else if(music){if(paused){Mix_ResumeMusic();started_ticks=SDL_GetTicks();paused=0;}else if(Mix_PlayingMusic()){base_position=get_position(base_position,started_ticks,0);Mix_PauseMusic();paused=1;}}continue;}
                if(screen!=SCREEN_DOWNLOADS&&e.jbutton.button==BUTTON_Y){screen=(screen==SCREEN_PLAYER)?SCREEN_MENU:SCREEN_PLAYER;continue;}
            }
        ...
        if(SDL_GetTicks()-last_battery_check>=5000){battery_percent=get_battery_percent();battery_charging=is_battery_charging();last_battery_check=SDL_GetTicks();}
        ...
        SDL_SetRenderDrawColor(renderer,15,15,20,255);SDL_RenderClear(renderer);
        ScreenContext screen_ctx={renderer,font,white,selected,gray,&screen,&running,&book_index,&track_index,&book_count,&track_count,book_names,book_paths,tracks,&music,&base_position,&started_ticks,&duration,track_durations,&book_duration,&paused,&last_save,&axis_y_lock,&axis_x_lock,&sleep_timer_active,&sleep_timer_minutes,&sleep_timer_end_ticks,storage_paths,&storage_path_count,book_roots,book_track_counts,&cpu_usage,&ram_usage,&cpu_temperature,&battery_percent,&battery_charging,&idle_timer_remaining_ms,do_shutdown};
        if(locked){draw_text(renderer,font,"Tastensperre",20,40,selected);draw_text(renderer,font,"Zum Entsperren:",20,100,white);int x=20;for(int i=0;i<UNLOCK_SEQUENCE_LEN;i++){const char *name=button_name(unlock_sequence[i]);SDL_Color col=i<unlock_progress?selected:white;draw_text(renderer,font,name,x,150,col);x+=70;}if(SDL_GetTicks()<unlock_wrong_flash_until)draw_text(renderer,font,"Falsche Taste",20,210,gray);}else{switch(screen){case SCREEN_MENU:menu_render(&screen_ctx);break;case SCREEN_TRACKS:tracks_render(&screen_ctx);break;case SCREEN_PLAYER:player_render(&screen_ctx);break;case SCREEN_SLEEP_TIMER:sleeptimer_render(&screen_ctx);break;case SCREEN_SYSTEM_INFO:systeminfo_render(&screen_ctx);break;case SCREEN_BUTTON_DEBUG:buttondebug_render(&screen_ctx);break;case SCREEN_SYSTEM_MENU:systemmenu_render(&screen_ctx);break;case SCREEN_DOWNLOADS:downloadbrowser_render(&screen_ctx);break;}}
        SDL_RenderPresent(renderer);SDL_Delay(10);
    }
    save_state();media_keys_close(&media_keys);if(music){Mix_HaltMusic();Mix_FreeMusic(music);}if(joy)SDL_JoystickClose(joy);if(font)TTF_CloseFont(font);SDL_DestroyRenderer(renderer);SDL_DestroyWindow(window);Mix_CloseAudio();TTF_Quit();SDL_Quit();return 0;
}
