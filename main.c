...
int main(int argc,char **argv)
{
 ...
 while(running){
  ...
  const char *mpris_album=(book_count>0&&book_index>=0&&book_index<book_count)?book_names[book_index]:"";
  const char *mpris_title=(track_count>0&&track_index>=0&&track_index<track_count)?tracks[track_index].name:"";
  ...
  mpris_bridge_update(&mpris,mpris_album,mpris_title,track_count>0?track_index+1:0,track_count,duration,mpris_pos,music!=NULL,paused,mpris_playing,(double)volume/MIX_MAX_VOLUME);
  MediaKeyAction media_actions[32];int media_action_count=media_keys_poll(&media_keys,media_actions,32);media_action_count+=mpris_bridge_poll(&mpris,media_actions+media_action_count,32-media_action_count);if(media_action_count>0){last_activity=SDL_GetTicks();idle_timer_remaining_ms=idle_timer_minutes>0?(Uint32)idle_timer_minutes*60000U:0U;idle_timer_last_tick=SDL_GetTicks();}
 ...
 save_state();mpris_bridge_close(&mpris);media_keys_close(&media_keys);if(music){Mix_HaltMusic();Mix_FreeMusic(music);}if(joy)SDL_JoystickClose(joy);if(font)TTF_CloseFont(font);SDL_DestroyRenderer(renderer);SDL_DestroyWindow(window);Mix_CloseAudio();TTF_Quit();SDL_Quit();return 0;
}
