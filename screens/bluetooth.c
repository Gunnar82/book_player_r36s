...
void bluetoothscreen_render(ScreenContext *c)
{
    ...
    if(message[0]&&(Sint32)((Uint32)message_until-now)>0)draw_text(c->renderer,c->font,message,35,SCREEN_H-65,c->selected);draw_text(c->renderer,c->font,"A: Aktion  X: Entfernen  Links/Rechts: Autoconnect  B: Zurueck",20,SCREEN_H-30,c->gray);
}
