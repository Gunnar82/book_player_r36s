...
static Uint32 last_refresh,last_sink_refresh;
static char sink_name[512]="";static int sink_volume=-1;
...
static void refresh(void){if(!bluetooth_adapter_powered()){device_count=0;selection=0;refresh_sink(1);return;}device_count=bluetooth_scan_paired_trusted(devices,BT_MAX_DEVICES);refresh_sink(1);if(!scanning&&selection>device_count+2)selection=device_count+2;}
...
void bluetoothscreen_reset(void){if(scanning)bluez_discovery_stop();scanning=0;selection=0;live_count=0;message[0]='\0';message_until=0;last_sink_refresh=0;
#ifdef BUILD_R36S
r36s_bluetooth_input_set_active(1);
#endif
bluetooth_log_if_changed();refresh();}
static void move(int delta){int max=scanning?(live_count>0?live_count-1:0):device_count+2;do{selection+=delta;if(selection<0)selection=max;if(selection>max)selection=0;}while(!scanning&&selection==2&&!sink_available());}
...
void bluetoothscreen_handle_event(ScreenContext *c,const SDL_Event *e)
{
    ...
    if(e->type==SDL_JOYBUTTONDOWN){int b=e->jbutton.button;
        ...
        if(selection==2&&b==BUTTON_DPAD_LEFT){change_sink_volume(-5);return;}
        if(selection==2&&b==BUTTON_DPAD_RIGHT){change_sink_volume(5);return;}
        if(selection>=3&&selection<device_count+3){BluetoothDevice *d=&devices[selection-3];if(b==BUTTON_R1){remove_device(d);return;}if(b==BUTTON_A){snprintf(bluetooth_device_mac,sizeof(bluetooth_device_mac),"%s",d->mac);if(bluetooth_save_config()!=0){set_message("Auswahl konnte nicht gespeichert werden");return;}set_message("Verbinde ...");int rc=bluetooth_connect_device(d->mac);refresh();set_message(rc==0?"Verbindung erfolgreich":"Verbindung fehlgeschlagen");return;}}
    }
...
void bluetoothscreen_render(ScreenContext *c)
{
    ...
    refresh_sink(0);draw_text(c->renderer,c->font,"Bluetooth",20,20,c->selected);char buf[256];snprintf(buf,sizeof(buf),"Autoconnect beim Start: < %s >",bluetooth_autoconnect?"Ein":"Aus");draw_text(c->renderer,c->font,buf,35,60,selection==0?c->selected:c->white);draw_text(c->renderer,c->font,"Geraete suchen",35,94,selection==1?c->selected:c->white);if(sink_available())snprintf(buf,sizeof(buf),"Ausgangspegel: < %d%% >",sink_volume);else snprintf(buf,sizeof(buf),"Ausgangspegel: nicht verfuegbar");draw_text(c->renderer,c->font,buf,35,128,sink_available()?(selection==2?c->selected:c->white):c->gray);int y=170;if(!bluetooth_adapter_present())draw_text(c->renderer,c->font,"Kein Bluetooth-Adapter vorhanden",35,y,c->gray);else if(!bluetooth_adapter_powered())draw_text(c->renderer,c->font,"Bluetooth ist ausgeschaltet",35,y,c->gray);else if(device_count==0)draw_text(c->renderer,c->font,"Keine gekoppelten Geraete",35,y,c->gray);
    int first=0;if(selection>8)first=selection-8;if(first>device_count-6)first=device_count>6?device_count-6:0;if(first<0)first=0;for(int i=first;i<device_count&&i<first+6;i++){BluetoothDevice *d=&devices[i];SDL_Color col=(selection==i+3)?c->selected:c->white;snprintf(buf,sizeof(buf),"%s%s%s",d->name,d->connected?" [verbunden]":"",!strcmp(d->mac,bluetooth_device_mac)?" *":"");draw_text(c->renderer,c->font,buf,35,y,col);draw_text(c->renderer,c->font,d->mac,55,y+22,c->gray);y+=46;}
    if(message[0]&&(Sint32)((Uint32)message_until-now)>0)draw_text(c->renderer,c->font,message,35,SCREEN_H-65,c->selected);draw_text(c->renderer,c->font,"A: Aktion  R1: Entfernen  Links/Rechts: Wert  B: Zurueck",20,SCREEN_H-30,c->gray);
}
