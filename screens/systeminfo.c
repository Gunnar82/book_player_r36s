...
static int build_lines(ScreenContext *c,InfoLine *lines){
    ...
    if(led_gpio>=0){snprintf(buf,sizeof(buf),"<  %d  >  %s%s",led_gpio,led_gpio_is_manual()?"Manuell":"Auto",led_gpio_available(led_gpio)?"":" / nicht verfuegbar");}
    else{snprintf(buf,sizeof(buf),"<  Auto  >  nicht erkannt");}
    ...
    if(sleep_timer_edit_minutes>=0){
        if(shown_sleep>0)snprintf(buf,sizeof(buf),"<  %d min  >  A: OK",shown_sleep);else snprintf(buf,sizeof(buf),"<  Aus  >  A: OK");
    }else if(*c->sleep_timer_active){
        Uint32 now=SDL_GetTicks();Uint32 remaining=(*c->sleep_timer_end_ticks>now)?(*c->sleep_timer_end_ticks-now):0;
        int rem_min=(int)(remaining/60000);int rem_sec=(int)((remaining/1000)%60);
    ...
    if(idle_timer_minutes>0)snprintf(buf,sizeof(buf),"<  %d min  >",idle_timer_minutes);else snprintf(buf,sizeof(buf),"<  Aus  >");
    add_line(lines,&n,"Idle-Timer",buf,0);
    if(shutdown_after_tracks>0)snprintf(buf,sizeof(buf),"<  %d Tracks  >",shutdown_after_tracks);else snprintf(buf,sizeof(buf),"<  Aus  >");
    ...
    add_line(lines,&n,"Config",get_storage_config_path(),0);
    ...
    for(int i=0;i<*c->storage_path_count;i++){
        if(c->storage_paths[i].available)format_free_space(c->storage_paths[i].path,buf,sizeof(buf));else snprintf(buf,sizeof(buf),"nicht verfuegbar");
...
static int find_repeat_line(InfoLine *lines,int count){for(int i=0;i<count;i++)if(!strcmp(lines[i].label,"Hoerspielende"))return i;return -1;}
static int find_shutdown_tracks_line(InfoLine *lines,int count){for(int i=0;i<count;i++)if(!strcmp(lines[i].label,"Shutdown nach Tracks"))return i;return -1;}
static int find_shutdown_end_line(InfoLine *lines,int count){for(int i=0;i<count;i++)if(!strcmp(lines[i].label,"Shutdown am Ende"))return i;return -1;}
...
static void toggle_downloads(void){downloads_enabled=!downloads_enabled;if(save_download_enabled()!=0)fprintf(stderr,"Einstellungen: Download-Status konnte nicht gespeichert werden.\n");}
...
void systeminfo_handle_event(ScreenContext *c,const SDL_Event *e){
    ...
    if(e->type==SDL_JOYBUTTONDOWN){int b=e->jbutton.button;
        if(b==BUTTON_B){led_set_test_mode(0);sleep_timer_edit_minutes=-1;page_offset=0;selected_line=1;*c->screen=SCREEN_SYSTEM_MENU;return;}
        ...
        int gl=find_led_gpio_line(lines,count),tl=find_led_test_line(lines,count),vl=find_volume_line(lines,count),bl=find_brightness_line(lines,count),sl=find_sleep_line(lines,count),il=find_idle_line(lines,count),rl=find_repeat_line(lines,count),stl=find_shutdown_tracks_line(lines,count),sel=find_shutdown_end_line(lines,count),dl=find_line(lines,count,"Downloads");
        if((b==BUTTON_A||b==BUTTON_DPAD_LEFT||b==BUTTON_DPAD_RIGHT)&&selected_line==dl){toggle_downloads();return;}
    ...
    if(e->type==SDL_JOYAXISMOTION){
        ...
        if(e->jaxis.axis==AXIS_X){
            int gl=find_led_gpio_line(lines,count),tl=find_led_test_line(lines,count),vl=find_volume_line(lines,count),bl=find_brightness_line(lines,count),sl=find_sleep_line(lines,count),il=find_idle_line(lines,count),rl=find_repeat_line(lines,count),stl=find_shutdown_tracks_line(lines,count),sel=find_shutdown_end_line(lines,count),dl=find_line(lines,count,"Downloads");
            if(selected_line==dl){if(!*c->axis_x_lock&&abs(e->jaxis.value)>AXIS_DEADZONE){toggle_downloads();*c->axis_x_lock=1;}}
            else if(selected_line==rl){if(!*c->axis_x_lock&&abs(e->jaxis.value)>AXIS_DEADZONE){toggle_repeat_book();*c->axis_x_lock=1;}}
            ...
            else if(selected_line==tl){if(!*c->axis_x_lock&&abs(e->jaxis.value)>AXIS_DEADZONE){led_set_test_mode(!led_test_mode());*c->axis_x_lock=1;}}
...
void systeminfo_render(ScreenContext *c){
    ...
    draw_text(c->renderer,c->font,"Einstellungen",20,20,c->selected);int y=TOP_Y;
