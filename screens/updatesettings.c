...
static void *update_download_worker(void *userdata)
{
    UpdateManifest manifest;
...
static int start_worker(void *(*worker)(void *))
{
    pthread_t thread;
...
static int start_update_check(void)
{
    ...
    if (start_worker(update_check_worker) != 0) {
        pthread_mutex_lock(&check_mutex);
        check_running = 0;
...
static int start_update_download(void)
{
    ...
    if (start_worker(update_download_worker) != 0) {
        pthread_mutex_lock(&check_mutex);
        download_running = 0;
        snprintf(check_status, sizeof(check_status), "Download konnte nicht gestartet werden");
        pthread_mutex_unlock(&check_mutex);
        return -1;
    }
    return 0;
...
static int build_rows(Row *rows)
{
    ...
    if (finished && result == 0 && check_manifest.version[0]) {
        ...
        newer = cmp > 0;
        if (cmp > 0) snprintf(available, sizeof(available), "Neu: %s", check_manifest.version);
        else if (cmp == 0) snprintf(available, sizeof(available), "Aktuell");
        else snprintf(available, sizeof(available), "Server: %s", check_manifest.version);
    } else {
        snprintf(available, sizeof(available), "--");
    }
    pthread_mutex_unlock(&check_mutex);

    rows[n++] = (Row){"Installiert", APP_VERSION, 0, ACTION_NONE};
    rows[n++] = (Row){"Update URL", update_base_url[0] ? update_base_url : "--", 0, ACTION_NONE};
    snprintf(tls_mode, sizeof(tls_mode), "%s", update_use_download_tls ? "Downloads" : "Eigenstaendig");
    rows[n++] = (Row){"TLS Quelle", tls_mode, 0, ACTION_NONE};
    rows[n++] = (Row){"TLS Peer", update_config_verify_peer() ? "Pruefen" : "Nicht pruefen", 0, ACTION_NONE};
    rows[n++] = (Row){"TLS Host", update_config_verify_host() ? "Pruefen" : "Nicht pruefen", 0, ACTION_NONE};
    rows[n++] = (Row){"CA Zertifikat", update_config_ca_cert()[0] ? update_config_ca_cert() : "System-CA", 0, ACTION_NONE};
    rows[n++] = (Row){"Client Zertifikat", update_config_client_cert()[0] ? update_config_client_cert() : "--", 0, ACTION_NONE};
    rows[n++] = (Row){"Client Key", update_config_client_key()[0] ? update_config_client_key() : "--", 0, ACTION_NONE};
...
{
    for (int i = 0; i < count; i++) if (rows[i].selectable) return i;
    return 0;
}
...
void updatesettings_handle_event(ScreenContext *c, const SDL_Event *e)
{
    ...
    if (e->type == SDL_JOYBUTTONDOWN) {
        ...
        if (b == BUTTON_DPAD_UP || b == BUTTON_DPAD_DOWN) {
            int direction = b == BUTTON_DPAD_UP ? -1 : 1;
            do {
                selection += direction;
    ...
    if (e->type == SDL_JOYAXISMOTION && e->jaxis.axis == AXIS_Y) {
        if (!*c->axis_y_lock && abs(e->jaxis.value) > AXIS_DEADZONE) {
            int direction = e->jaxis.value < 0 ? -1 : 1;
            do {
                selection += direction;
                if (selection < 0) selection = count - 1;
                if (selection >= count) selection = 0;
...
void updatesettings_render(ScreenContext *c)
{
    ...
    draw_text(c->renderer, c->font, "A: Ausfuehren   B: Zurueck", 20, SCREEN_H - 30, c->gray);
}
