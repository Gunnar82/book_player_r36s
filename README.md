...
`develop-0.2` basiert weiterhin auf dem stabilen 0.1.14-Quellstand. Die 0.2-Änderungen liegen unter `patches/` als Patch-Stack. `01-core.patch` enthält ältere Kontextzeilen und wird deshalb tolerant angewendet; die wenigen Core-Stellen in `main.c`, die wegen späterer 0.1-Änderungen nicht mehr direkt passen, ergänzt das Dockerfile idempotent. Danach prüft der Build explizit die benötigten Download-Deklarationen, Screen-IDs und Handler, bevor kompiliert wird.
