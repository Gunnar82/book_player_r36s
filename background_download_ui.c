...
{
    char line[768],rate[32],folder[120];double pct=total_percent(s);format_rate(rate,sizeof(rate),s->rate_bps);format_tail(folder,sizeof(folder),s->folder[0]?s->folder:"/",96);
    draw_text(c->renderer,c->font,"Download aktiv",20,20,c->selected);
    snprintf(line,sizeof(line),"Datei %d / %d: %s",s->file_index,s->file_count,s->filename[0]?s->filename:"...");draw_text(c->renderer,c->font,line,20,72,c->white);
    snprintf(line,sizeof(line),"Gesamt: %.1f / %.1f MB  (%.0f %%)",s->total_now/1048576.0,s->total_size/1048576.0,pct);draw_text(c->renderer,c->font,line,20,122,c->white);
    SDL_Rect bar={20,160,580,12};SDL_SetRenderDrawColor(c->renderer,70,70,70,255);SDL_RenderFillRect(c->renderer,&bar);SDL_Rect fill=bar;fill.w=(int)(bar.w*pct/100.0);SDL_SetRenderDrawColor(c->renderer,230,210,70,255);SDL_RenderFillRect(c->renderer,&fill);
    snprintf(line,sizeof(line),"Ordner: %s",folder);draw_text(c->renderer,c->font,line,20,210,c->gray);snprintf(line,sizeof(line),"Downloadrate: %s",rate);draw_text(c->renderer,c->font,line,20,252,c->gray);draw_text(c->renderer,c->font,"Y: Im Hintergrund   B: Abbrechen",20,SCREEN_H-35,c->gray);
}
