...
static int parse_listing(const char *xml,RemoteEntry *entries,int max_entries)
{
    ...
    while(*p&&n<max_entries){
        ...
        memcpy(tag,start,taglen);tag[taglen]='\0';
...
int remote_fetch_listing(const char *relative_path,RemoteEntry *entries,int max_entries,char *error,size_t error_size)
{
    ...
    if(!download_base_url[0]){
        snprintf(error,error_size,"base_url fehlt in config.ini");
        return -1;
    }
    if(!ensure_curl_global()){
        snprintf(error,error_size,"libcurl konnte nicht initialisiert werden");
        return -1;
    }
    ...
    if(rc!=CURLE_OK){
        snprintf(error,error_size,"Netzwerk: %s",curl_easy_strerror(rc));
        free(mem.data);return -1;
    }
                          ...
                          DownloadProgressFn progress,void *userdata,char *error,size_t error_size)
{
    ...
    if(relative_path&&relative_path[0])
        snprintf(local_dir,sizeof(local_dir),"%s/%s",download_target_path,relative_path);
    else
        snprintf(local_dir,sizeof(local_dir),"%s",download_target_path);

    if(make_dirs(local_dir)!=0){
        snprintf(error,error_size,"Zielverzeichnis kann nicht angelegt werden: %s",local_dir);
        return -1;
    }
    ...
    for(int i=0;i<entry_count;i++){
        ...
        char dir_url[4096],url[4608],local[2600],part[2620];
        if(append_encoded_path(curl,relative_path,dir_url,sizeof(dir_url))!=0){
            snprintf(error,error_size,"Ungueltiger Remote-Pfad");curl_easy_cleanup(curl);return -1;
        }
        ...
        if(rename(part,local)!=0){
            unlink(part);
            snprintf(error,error_size,"Kann fertige Datei nicht umbenennen");
            curl_easy_cleanup(curl);return -1;
        }
        if(entries[i].size>0)completed+=entries[i].size;
    }
