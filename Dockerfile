...
RUN grep 'APP_VERSION "0.3.1"' config.h && \
...
RUN gcc -DBUILD_R36S -o hoerspiel_player \
    main.c state.c backlight.c battery.c battery_bluez.c led.c scanner.c audio.c ui.c \
    storage.c systemstats.c media_keys.c media_feedback.c app_log.c input_config.c streaming.c qr_render.c mpris_bridge.c bluetooth.c hfp_gateway.c pbap_phonebook.c download.c \
    ...
    $(pkg-config --cflags sdl2 SDL2_mixer SDL2_ttf libcurl libsystemd libqrencode) \
    $(pkg-config --libs sdl2 SDL2_mixer SDL2_ttf libcurl libsystemd libqrencode)
